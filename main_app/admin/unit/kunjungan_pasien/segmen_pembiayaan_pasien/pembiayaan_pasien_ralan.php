<?php
  ...
  $tahun = date('Y'); 
  $whereClause = "YEAR(r.tgl_registrasi) = '$tahun' AND r.status_lanjut = 'Ralan'"; 
...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <?php if ($id =='A09') { ?>
          <h1 style="font-size: 19px;">METODE PEMBAYARAN PASIEN RALAN BEDASARKAN UMUM</h1>
        <?php } else if ($id =='BPJ')  { ?>
          <h1 style="font-size: 19px;">METODE PEMBAYARAN PASIEN RALAN BEDASARKAN BPJS</h1>
        <?php } else if ($id =='B1')  { ?>
          <h1 style="font-size: 19px;">METODE PEMBAYARAN PASIEN RALAN BEDASARKAN ASURANSI</h1>
        <?php } else { ?>
          <h1 style="font-size: 19px;">METODE PEMBAYARAN PASIEN RALAN</h1>
        <?php } ?>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="main_admin_app.php?unit=beranda">Home</a></li>
          <li class="breadcrumb-item active">Segmen Pembayaran Pasien Ralan</li>
        </ol>
      </div>
    </div>
  </div>
        ...
        <div class="card-header">
              <div class="card-tools" style="float: left; text-align: left;">
               <select class="form-control select2" style="min-width: 250px;" onChange="document.location.href=this.options[this.selectedIndex].value;" id="jenis" name="jenis">
                    <option selected="selected" value="?unit=bayar_ralan"> Silahkan Pilih .....</option>
                    <option value="?unit=bayar_ralan&action=datagrid&id=BPJ" <?php if ($id == 'BPJ') {echo "selected";}?>>Pasien BPJS</option>
				            <option value="?unit=bayar_ralan&action=datagrid&id=A09" <?php if ($id == 'A09') {echo "selected";}?>>Pasien Umum</option>
                    <option value="?unit=bayar_ralan&action=datagrid&id=B1" <?php if ($id == 'B1') {echo "selected";}?>>Asuransi / Pihak ke - III</option>
               </select>
              </div>
          ...
          <div class="card-body">
              <div class="table-responsive">
                <table id="example" class="table table-bordered table-striped">
                  ...
                  <tbody style="background:rgb(143, 143, 143)">
          ...
          <div class="modal-dialog">
               <div class="modal-content">
                    ...
                    <div class="modal-body" align = "left">
                         <form role="form" enctype="multipart/form-data" method="post" action="laporan/lap_bayar_ralan.php?action=tanggal" target="blank">
                              <div class="row">
